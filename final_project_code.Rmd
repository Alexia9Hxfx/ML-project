...
roc_plot <- ggroc(knn_roc, legacy.axes = TRUE)+
  labs(title = "ROC Curve of KNN in Training Data Prediction", subtitle = auc_label_knn)+
...
#data_1178$random <- runif(625) # add random variable

train_ctrl <- trainControl(method = "cv",number = 3) # method = "boot", number = 20, returnResamp = 'final', savePredictions = 'final'
# train rf to measure conversion probability
fit_rf <- train(factor(target) ~ Spent + factor(interest) + genderM + `age30-34` + `age35-39` + `age40-44` + `age45-49`, 
                method = "rf", 
                data = data_1178,
...
p_rf <- predict(fit_rf, type="raw", newdata = data_1178)
tab <- table(Predicted=p_rf, Actual=data_1178$target)
confusionMatrix(tab)

# add conversion probability variable
p_rf <- predict(fit_rf, type="prob", newdata = data_1178)
data_1178 <- cbind(data_1178,p_rf$`1`)
colnames(data_1178)[17] <- "predict"
...
ggplot(
  ...
  labs(title = 'Random Forest Model Variable Importance', subtitle="please notice the importance of a random variable")

# Create ROC curve
roc_curve <- roc(data_1178$target, data_1178$predict)
...
```{r}
# Example AUC values
auc_values <- c("Logistic Regression" = 0.8701, "Random Forest" = 0.8448, "Neural Network" = 0.7097, "Gradient Boosting" = 0.8422)
